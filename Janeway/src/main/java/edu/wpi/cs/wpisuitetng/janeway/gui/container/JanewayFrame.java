...
public class JanewayFrame extends JFrame {
	...
	private JanewayFrame(List<IJanewayModule> modules) {
		...
		setMinimumSize(new Dimension(800, 600)); // minimum window size is 800 x
													// 600
		...
		JanewayKeyEventDispatcher keyEventDispatcher = new JanewayKeyEventDispatcher(
				this, modules);
	...
	public static JanewayFrame getInstance() {
		if (instance == null) {
			throw new RuntimeException(
					"You must call the initialize method of JanewayFrame before calling getInstance!");
		}
		return instance;
	}
	 ...
	 * 
	 * @param keyEventDispatcher
	 *            the current key event dispatcher
	 */
	private void addGlobalKeyboardShortcuts(
			JanewayKeyEventDispatcher keyEventDispatcher) {

		// control + page down: switch to the module tab to the right
		keyEventDispatcher.addGlobalKeyboardShortcut(new KeyboardShortcut(
				KeyStroke.getKeyStroke("control PAGE_DOWN"),
				new AbstractAction() {
					@Override
		...
		keyEventDispatcher.addGlobalKeyboardShortcut(new KeyboardShortcut(
				KeyStroke.getKeyStroke("control PAGE_UP"),
				new AbstractAction() {
					@Override
					public void actionPerformed(ActionEvent event) {
