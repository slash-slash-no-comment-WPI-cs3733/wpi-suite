...
public class DragImage extends AbstractComponentDecorator {
	 ...
	 *            offset of the image from the cursor
	 */
	public DragImage(JComponent dragSource, Point screenLocation, Icon icon,
			Point imageOffset) {
	 ...
	 */
	protected DragImage(JComponent dragSource, JLayeredPane root,
			...
			for (int i = 0; i < frames.length; i++) {
				...
				if (frame instanceof RootPaneContainer && frame.isShowing()
					...
					JLayeredPane p = ((RootPaneContainer) frame)
							.getLayeredPane();
					DragImage slave = new DragImage(dragSource, p,
							screenLocation, icon, imageOffset, false);
					ghosts.add(slave);
	...
	public void setCursor(Cursor cursor) {
		...
		for (Iterator i = ghosts.iterator(); i.hasNext();) {
			DragImage slave = (DragImage) i.next();
