...
public class Requirement extends AbstractModel {
	...
	public void setRelease(String release) {
		if (!release.equals(this.release) && !wasCreated) {
			...
			if (this.release.isEmpty())
				...
				message = ("Release Number changed from '" + this.release
						+ "' to '" + release + "'");
			this.history.add(message);
		}
	...
	public void setStatus(RequirementStatus status) {
		if ((status != this.status) && (!wasCreated)) {
			String message = ("Status changed from '" + this.status + "' to '"
					+ status + "'");
			this.history.add(message);
		}
	...
	public void setIteration(String newIterationName) {
		...
		// TODO: Do we really need these to through null pointer exceptions if
		// the iterations don't exist? Why not use the controller to prevent
		// this instead
		// Iteration oldIteration =
		// IterationModel.getInstance().getIteration(curIter);
		// Iteration newIteration =
		// IterationModel.getInstance().getIteration(newIterationName);

		if (!this.iteration.equals(newIterationName) && (!wasCreated)) {
			// create the transaction history
			this.history.add("Moved from '" + this.iteration + "' to '"
					+ newIterationName + "'");
		}

		// update status as needed
		if ((this.status.equals(RequirementStatus.NEW) || this.status
				.equals(RequirementStatus.OPEN))
				&& !newIterationName.equals("Backlog")) {
			this.setStatus(RequirementStatus.INPROGRESS);
		}

		if (this.status.equals(RequirementStatus.INPROGRESS)
				&& newIterationName.equals("Backlog")) {
			if (wasCreated) {
	...
	/**
	 * Setter for parentID Assign the parent ID for this requirement
	 * 
	 * @param parentID
	 *            ID of the parent @throws Exception if the parent is an
	...
	public void setParentID(int parentID) throws Exception {
		if (parentID == -1
			...
			// " was added as a Child Requirement");
		} else {
	 ...
	 * 
	 * @param parentId
	 *            The ID of the parent requirement @return true if the parent is
	 *            an ancestor
	 */
	 ...
	 * 
	 * @param childId
	 *            ID of the child @return true if it is an ancestor of the child
	 */
