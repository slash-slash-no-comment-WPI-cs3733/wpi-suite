 ...
 * contacting their respective entity managers
 * 
 * eagerly initialized, the instance of this class is thread safe, provided all
 * methods are thread safe
 * 
 * Remember Thread Safety All methods must be thread safe
 */
public class ManagerLayer {
	...
	private static final Logger logger = Logger.getLogger(ManagerLayer.class
			.getName());
	...
	private ManagerLayer() {
		...
		map.put("taskmanager" + "task", new GenericEntityManager<TaskModel>(
				data, TaskModel.class));
		map.put("taskmanager" + "stage", new GenericEntityManager<StageModel>(
				data, StageModel.class));
		...
		try {
			ssid = sessions.createSession((User) map.get("coreuser").getEntity(
					null, "admin")[0]);
	 ...
	 * initializes the database initializes the JSON serializer
	 * 
	 * Accepts a map that will be used instead of the automatically generated
	 * map
	 * 
	...
	/**
	 * Returns the single ManagerLayer instance This is a test managerlayer with
	 * dummy EntityManagers This call is not a true singleton call, and will
	 * return a new managerlayer everytime it is accessed
	 * 
	 * @return ManagerLayer
	 */
	...
	/**
	 * read() Reads the WPISuite cookie and returns the session associated with
	 * it in JSON form String args[] - {module,model,identifier}
	 * 
	 * @param args
	 *            - a string array of the parameters, where args[length-1] ==
	 *            null
	 * @return a JSON String representing the requested data
	 */
	public synchronized String read(String[] args, Cookie[] cook)
			throws WPISuiteException {
	...
	/**
	 * create() Creates a JSON representation of the model created from the
	 * content passed in in the arguments. * String args[] -
	 * {module,model,identifier}
	 * 
	 * @param args
	 *            - a string array of the parameters
	 * @param content
	 *            - the content of the create request
	 * @return a JSON String of the newly created data if successful, null
	 *         otherwise
	 */
	public synchronized String create(String[] args, String content,
			Cookie[] cook) throws WPISuiteException {
		Session s = getSessionFromCookies(cook);
	 ...
	 *            - A string array of the parameters
	 * @param content
	 *            - a JSON String of the content to update
	...
	/**
	 * delete * String args[] - {module,model,identifier} Deletes a model
	 * identified by the cookie and content of the model.
	 * 
	 * @param args
	 *            - A String array of the parameters
	 ...
	 * 
	 */
	public String advancedGet(String[] args, Cookie[] cook)
			throws WPISuiteException {
		Session s = getSessionFromCookies(cook);
	 ...
	 * Advanced Put
	 * 
	 * ********************** A note about advanced put. the content body should
	 * not contain any line breaks. only the first line will be passed through
	 * to the function. **********************
	 * 
	 * does the same thing as advanced GET except that it also forwards as well
	 * as the path arguments
	 * 
	 * @param args
	 *            - the path arguments of the request
	 * @param content
	 *            - the content body of the request
	 * @param cook
	 *            - the cookie sent along with the request
	 ...
	 * ADvanced Post
	 * 
	 * ********************** A note about advanced post. the content body
	 * should not contain any line breaks. only the first line will be passed
	...
	private Session getSessionFromCookies(Cookie[] cook)
